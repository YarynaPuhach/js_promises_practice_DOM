...
const thirdPromise = new Promise((resolve)=>{
    ...
    document.addEventListener("mousedown", (click)=>{
        if (click.button === 0) leftClicked = true;
        if (click.button === 2) rightClicked = true;
        if (leftClicked && rightClicked) resolve("Third promise was resolved");
    });
});
firstPromise.then(()=>createNotification("First promise was resolved", "success")).catch(()=>createNotification("First promise was rejected", "error"));
secondPromise.then(()=>{
